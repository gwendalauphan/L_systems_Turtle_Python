...
class MyTest(unittest.TestCase):
    ...
    def test_fileIsValid(self):                                                 #test de la fonction de vérification de validité du fichier d'entrée (buisson.txt est valide et fougere_sans_taille.txt ne l'est pas)
        with open("tests/buisson.txt", "r") as f:
            self.assertTrue(fileIsValid(f.read()))
        with open("tests/fougere_sans_taille.txt", "r") as f:
